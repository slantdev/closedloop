...
add_action('after_setup_theme', 'closedloop_setup');

/**
 * Enqueue theme assets.
 */
function closedloop_enqueue_scripts()
{
  $theme = wp_get_theme();
  ...
  wp_enqueue_script('hover-intent', closedloop_asset('js/jquery.hoverIntent.js'), array('jquery'), '1.10.2');
  wp_enqueue_script('closedloop', closedloop_asset('js/app.js'), array('jquery'), $theme->get('Version'));
}
add_action('wp_enqueue_scripts', 'closedloop_enqueue_scripts');

add_action('admin_enqueue_scripts', 'closedloop_admin_styles');
function closedloop_admin_styles()
{
  global $pagenow;
 ...
 * @return string
 */
function closedloop_asset($path)
{
 ...
 * @return array
 */
function closedloop_nav_menu_add_li_class($classes, $item, $args, $depth)
{
...
add_filter('nav_menu_css_class', 'closedloop_nav_menu_add_li_class', 10, 4);
 ...
 * @return array
 */
function closedloop_nav_menu_add_submenu_class($classes, $args, $depth)
{
...
add_filter('nav_menu_submenu_css_class', 'closedloop_nav_menu_add_submenu_class', 10, 3);
